document.addEventListener('DOMContentLoaded', function() {
    const ventasRopa = JSON.parse(localStorage.getItem('ventasRopa')) || [];
    ...
    function filterVentas() {
        ...
        const filteredVentas = ventasRopa.filter(venta => {
            return (
    ...
    function editVenta(index) {
        ...
        if (newFecha && newProducto && newNit && newPrecio && newCantidad) {
            ...
            displayVentas(ventasRopa);
        }
    }

    function deleteVenta(index) {
        if (confirm('¿Estás seguro de que deseas eliminar esta venta?')) {
            ventasRopa.splice(index, 1); // Elimina la venta del array
            localStorage.setItem('ventasRopa', JSON.stringify(ventasRopa)); // Actualiza el localStorage
            displayVentas(ventasRopa); // Refresca la tabla
        }
    }
